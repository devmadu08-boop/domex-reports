...
function OutstandingEntry({ report, updateStatus, calculated }) {
  return <div className="mt-4 overflow-x-auto rounded-2xl border border-blue-100 bg-white"><table className="audit-input-table min-w-[920px]"><thead><tr><th>Description</th><th>Cash Bills</th><th>Cash Amount</th><th>COD Bills</th><th>COD Amount</th><th>E-Com Bills</th><th>E-Com Amount</th></tr></thead><tbody>{AUDIT_STATUS_ROWS.map(([key, label, group]) => <tr key={key} className={group === "A" ? "audit-row-a" : "audit-row-b"}><th>{label}</th>{["cashBills","cashAmount","codBills","codAmount","ecomBills","ecomAmount"].map((metric) => <td key={metric}><input type="number" min="0" step={metric.endsWith("Amount") ? "0.01" : "1"} value={report.statuses[key][metric] || ""} onChange={(event) => updateStatus(key, metric, event.target.value)} /></td>)}</tr>)}</tbody><tfoot><AuditTotalRow label="Total A" values={calculated.totalA} /><AuditTotalRow label="Total B" values={calculated.totalB} /><AuditTotalRow label="Difference A-B" values={calculated.difference} /></tfoot></table></div>;
}
function AuditTotalRow({ label, values }) { return <tr><th>{label}</th>{["cashBills","cashAmount","codBills","codAmount","ecomBills","ecomAmount"].map((metric) => <td key={metric}>{metric.endsWith("Amount") ? money(values[metric]) : count(values[metric])}</td>)}</tr>; }
